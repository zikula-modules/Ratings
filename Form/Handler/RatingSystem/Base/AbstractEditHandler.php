...
namespace Paustian\RatingsModule\Form\Handler\RatingSystem\Base;

use Paustian\RatingsModule\Form\Handler\Common\EditHandler;
use Paustian\RatingsModule\Form\Type\RatingSystemType;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    {
        $this->objectType = 'ratingSystem';
        $this->objectTypeCapital = 'RatingSystem';
        $this->objectTypeLower = 'ratingsystem';
        ...
        if ('create' == $this->templateParameters['mode']) {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->requestStack->getCurrentRequest()->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the rating system yet as other items are required which must be created before!'));
                $logArgs = ['app' => 'PaustianRatingsModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType];
    ...
    {
        return $this->formFactory->create(RatingSystemType::class, $this->entityRef, $this->getFormOptions());
    }
    ...
    protected function getFormOptions()
    {
        $options = [
            ...
            'allow_moderation_specific_creator' => $this->variableApi->get('PaustianRatingsModule', 'allowModerationSpecificCreatorFor' . $this->objectTypeCapital, false),
            'allow_moderation_specific_creation_date' => $this->variableApi->get('PaustianRatingsModule', 'allowModerationSpecificCreationDateFor' . $this->objectTypeCapital, false),
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of rating system area
        $codes[] = 'userIndex';
        // admin index page of rating system area
        $codes[] = 'adminIndex';
        // user list of rating systems
        $codes[] = 'userView';
        // admin list of rating systems
        $codes[] = 'adminView';
        // user list of own rating systems
        $codes[] = 'userOwnView';
        // admin list of own rating systems
        $codes[] = 'adminOwnView';
    ...
    protected function getDefaultReturnUrl(array $args = [])
    {
        ...
        // redirect to the list of ratingSystems
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ('create' == $this->templateParameters['mode']) {
                    $message = $this->__('Done! Rating system created.');
                } else {
                    $message = $this->__('Done! Rating system updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Rating system deleted.');
                break;
            default:
                $message = $this->__('Done! Rating system updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'PaustianRatingsModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'rating system', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
