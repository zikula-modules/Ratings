...
(function ($) {
    ...
    class RatingSetter {
        ...
        setRating(evt) {
            ...
            if (user === "1"){
                window.alert("You must be logged in to set a rating");
                return;
            }
            this.sendAjax(
                ...
                {"success": this.updateRating.bind(this), method: 'POST'}
            );
        }
        ...
        updateRating(result, textStatus, jqXHR) {
            ...
            $(this.$averageNum).text("(" + Math.round(result.avgData.average *10)/10 + " of " + result.modVars.ratingScale + ")");
