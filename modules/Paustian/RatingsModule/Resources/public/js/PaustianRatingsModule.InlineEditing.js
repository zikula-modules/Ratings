...
function paustianRatingsInitInlineEditingButtons(objectType, alias, idPrefix, inputType, createUrl) {
    ...
    if (jQuery('#' + createButtonId).length < 1) {
        ...
        createButton = jQuery('<a>', {
            ...
            class: 'btn btn-default paustianratingsmodule-inline-button'
        }).append(
...
function paustianRatingsCloseWindowFromInside(idPrefix, itemId, formattedTitle, searchTerm) {
    ...
    jQuery.each(window.parent.paustianRatingsInlineEditHandlers, function (key, editHandler) {
        ...
        window.parent.paustianRatingsSimpleAlert(anchorElement, window.parent.Translator.__('Information'), window.parent.Translator.__('Action has been completed.'), 'actionDoneAlert', 'success');
