 ...
 */
function paustianRatingsCreateInlineEditingWindowInstance(containerElem) {
    var newWindowId;
 ...
 */
function paustianRatingsInitInlineEditingWindow(objectType, idPrefix, containerId, inputType) {
    ...
    jQuery.each(paustianRatingsInlineEditHandlers, function (key, editHandler) {
        ...
        editHandler.windowInstanceId = paustianRatingsCreateInlineEditingWindowInstance(jQuery('#' + containerId));
    });
    ...
    newEditHandler = {
        ...
        inputType: inputType,
        windowInstanceId: paustianRatingsCreateInlineEditingWindowInstance(jQuery('#' + containerId))
    };

    // add it to the list of edit handlers
    paustianRatingsInlineEditHandlers.push(newEditHandler);
}

/**
 * Creates a link for editing an existing item using inline editing.
 */
function paustianRatingsCreateInlineEditLink(objectType, idPrefix, elemPrefix, itemId) {
 ...
 */
function paustianRatingsInitInlineEditLink(objectType, idPrefix, elemPrefix, itemId, inputType) {
    jQuery('#' + elemPrefix + 'Edit').click(function (event) {
        event.preventDefault();
        paustianRatingsInitInlineEditingWindow(objectType, idPrefix, idPrefix + 'Reference_' + itemId + 'Edit');
    });
}
 ...
 */
function paustianRatingsDetermineInputReference(objectType, alias, idPrefix, inputType, targetWindow) {
    var inputPrefix, inputIdentifier, inputField;

    // determine reference to input element
    inputPrefix = targetWindow.jQuery('.paustianratings-edit-form').first().attr('name');
    inputField = null;
 ...
 */
function paustianRatingsInitInlineEditingButtons(objectType, alias, idPrefix, inputType, createUrl) {
    var inputReference, createButtonId, createButton, itemIds, itemIdsArr;

    inputReference = paustianRatingsDetermineInputReference(objectType, alias, idPrefix, inputType, window);
    ...
    if (jQuery('#' + createButtonId).length < 1) {
        ...
        createButton = jQuery('<a>', {
            ...
            class: 'btn btn-secondary paustianratingsmodule-inline-button'
        }).append(
    ...
    createButton.click(function (event) {
        event.preventDefault();
        paustianRatingsInitInlineEditingWindow(objectType, idPrefix, createButtonId, inputType);
    });
    ...
    jQuery.each(itemIdsArr, function (key, existingId) {
        ...
        if (existingId) {
            ...
            jQuery('#' + elemPrefix).click(function (event) {
                event.preventDefault();
                paustianRatingsInitInlineEditingWindow(objectType, idPrefix, elemPrefix, inputType);
            });
        }
    });
}
 ...
 */
function paustianRatingsCloseWindowFromInside(idPrefix, itemId, formattedTitle, searchTerm) {
    ...
    jQuery.each(window.parent.paustianRatingsInlineEditHandlers, function (key, editHandler) {
        ...
        inputReference = paustianRatingsDetermineInputReference(editHandler.objectType, editHandler.alias, idPrefix, inputType, window.parent);
        if (null === inputReference || null === inputReference.field) {
            return;
        }
        ...
        window.parent.paustianRatingsSimpleAlert(anchorElement, window.parent.Translator.trans('Information'), window.parent.Translator.trans('Action has been completed.'), 'actionDoneAlert', 'success');
...
jQuery(document).ready(function () {
    if (jQuery('#inlineRedirectParameters').length > 0) {
        ...
        paustianRatingsCloseWindowFromInside(
